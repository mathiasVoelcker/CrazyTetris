...
public static class MathGridDomain
{
    ...
    public static List<Block> CheckEquationsHorizontal(Transform[,] Grid)
    {
        ...
        for (int l = 0; l < LINES; l++)
        {
            ...
            for (int c = 0; c < COLUMNS; c++)
            {
                ...
                else if (MathTetrisSpawner.EqOperators.Any(x => x.ToString() == block.GetText()))
                {
            ...
            if (
                ...
                && !MathTetrisSpawner.EqOperators.Any(x => x.ToString() == equation.OrderBy(y => y.Column).Last().BlockObject.GetText()) //if the end of the equation is not a operator
